...
import gitlab
import gitlab.base
from gitlab.testing.docker import *  # noqa
...
def reset_gitlab(gl: gitlab.Gitlab) -> None:  # noqa C901
    ...
    logging.info("Waiting 2 seconds so stuff can be deleted...")
    time.sleep(2)
...
def set_token(container: str, docker_assets_dir: Path, gitlab_url: str) -> str:
    ...
    if gitlab_token_path.exists():
        logging.info("Found saved token. Checking if we can login")
        saved_token = gitlab_token_path.read_text()
        instance = gitlab.Gitlab(gitlab_url, private_token=saved_token)
        with contextlib.suppress(gitlab.GitlabAuthenticationError):
            instance.auth()
        if instance.user is not None:
            logging.info("That seems to have worked")
            return saved_token
    ...
    gitlab_token_path.write_text(output)
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--keep-containers",
        action="store_true",
        help="Only stop containers after testing",
    )
...
@pytest.fixture(scope="session")
def gitlab_config(gitlab_url: str, gitlab_token: str, temp_dir: Path):
